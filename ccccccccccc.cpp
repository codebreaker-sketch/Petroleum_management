...
int main()
{
    ...
    while (true)
    {
        ...
        if (!isLoggedIn)
        {
            ...
            {
                cout<<red << "\t\t\t\t Sorry, invalid choice! Please try again." <<reset<< endl;
                 SetConsoleTextAttribute(h, 11);
                continue; // Show the menu again
            }
            if (cin.gcount() > 1 || (loginChoice != USER_LOGIN && loginChoice != ADMIN_LOGIN && loginChoice != EXIT_PROGRAM))
            {
                cout <<red<< "\t\t\t\t Invalid choice! Please enter a single digit." <<reset<< endl;
                 SetConsoleTextAttribute(h, 11);
        ...
        while (isLoggedIn && displayUserMenu)
        {
            ...
            {
                cout << red<<"\t\t\t Invalid choice! Please enter a single digit between 1 and 4." <<reset<< endl;
                 SetConsoleTextAttribute(h, 11);
                cin.clear(); // Clear error state
